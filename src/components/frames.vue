<template>
  <div class="app" :style="{ paddingTop: statusBarHeight + 'px', height: navHeight + 'px' }">
    <div class="headArea" :style="{ height: headAreaHeight + 'px', marginTop: headAreaMarTop + 'px' }">
      <div class='leftBox'>
        <span @click="show_pro()">{{listCity}}<img v-if="type == true" src="../../static/images/dropdown1.png"/><img v-else src="../../static/images/dropdown.png"/></span>
        <p>{{titleName}}</p>
<!--        <p>平安鼎</p>-->
      </div>
      <div class='rightBox' :style="{ width: 10 + 'px' }">
      </div>
    </div>

    <!-- 弹窗 -->
    <div class="modalMask" v-if="isModel" @click="close()"></div>
    <div class="modalDialog" v-if="changeModel"  :style="{top:statusBarHeight + navHeight + 'px'}" >
      <div class="modalContent" >
        <p class="text" v-for="(item,index) in list" :key="index" @click="select(item)">
          <img  v-if="item.choose == false " src="../../static/images/Blankbox.png"/>
  ...
  export default {
    ...
    onReady: function () {
      ...
      if (this.info.system.substring(0, 3) == "iOS") {
        ...
        this.headleftBoxW = this.info.windowWidth - this.pixelRatio * 100 / this.pixelRatio;
        this.headRightBoxW = this.pixelRatio * 100 / this.pixelRatio;
        this.containersHeight = this.info.windowHeight - (this.statusBarHeight + this.pixelRatio * 40 / this.pixelRatio);
      } else {
        this.navHeight = this.pixelRatio * 45 / this.pixelRatio;
        this.headAreaMarTop = this.pixelRatio * 2.5 / this.pixelRatio;
        this.headleftBoxW =  this.info.windowWidth - this.pixelRatio * 110 / this.pixelRatio;
        this.headRightBoxW = this.pixelRatio * 110 / this.pixelRatio;
        this.containersHeight = this.info.windowHeight -  (this.statusBarHeight + this.pixelRatio * 45 / this.pixelRatio);
      }
    },
