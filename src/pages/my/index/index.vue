  ...
  <div class="home">
    ...
    <div class="main">
        ...
        <ul>
          ...
          <li class="attrsystem">
            ...
            <div>
              ...
              <!-- <span class="ct">{{info.unit ? info.unit : ''}}</span> -->
              <picker
                :range="info.AttrSystems"
                :value="index2"
                @change="bindPickerChange"
              >
                <span class="ct">{{
                  info.AttrSystem ? info.AttrSystem : "请选择归属系统"
                }}</span>
              </picker>
              <img src="../../../../static/my/line.png" />
            </div>
      ...
      <div class="table" v-if="integral == 0">
        <ul>
          <li class="attrsystem">
            <img src="../../../../static/my/myaddr.png" />
            <div>
              <p>当前区域<span></span></p>
              <span>：</span>
              <picker
                range-key="nm"
                :range="info.Addressarea"
                :value="index3"
                @change="ChangeAddress"
              >
                <span class="ct">{{
                  info.myaddr ? info.myaddr : "请选择当前区域"
                }}</span>
              </picker>
              <img src="../../../../static/my/line.png" />
            </div>
      ...
      <div class="table" v-if="permission.addTask || permission.auditTask">
        <ul>
          <li
            v-if="permission.addTask"
            @click="toPage('/pages/task/publish/main')"
          >
          ...
          <li
            v-if="permission.auditTask"
            @click="toPage('/pages/task/approve/main')"
          >
    ...
    <div class="modalDialog" v-if="changeModel">
      <div class="modalContent">
        ...
        <p v-if="type == 2" class="remind">
          注销账号将删除本人及从属的所有信息，并不可恢复。是否确认注销{{
            info.realNm
          }}
          {{ info.mob }}的账号？
        </p>
...
export default {
  ...
  methods: {
    ...
    bindPickerChange(e) {
      ...
      // 提交修改
      let obj = {};
      obj.id = wx.getStorageSync("userInfo").subId;
      obj.AttrSystem = this.info.AttrSystem;
      console.log("obj", obj);
    ...
    ChangeAddress(e) {
      ...
      this.$set(this.info, "myaddr", this.info.Addressarea[this.index3].nm);
      // 提交修改
      let obj = {};
      obj.id = wx.getStorageSync("userInfo").subId;
      obj.myaddr = this.info.Addressarea[this.index3].addrCd;
      console.log("obj", obj);
      this.api.upd(JSON.stringify(obj));
      // 不同区域首页显示不同内容
    ...
    async getArea() {
      ...
      this.$set(this.info, "Addressarea", data);
    },
    //授权
    loginBtnClick(e) {
      let that = this;
