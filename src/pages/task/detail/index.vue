...
<!--      </li>-->
      ...
      <li>
        <p class="hd">链接地址</p>
        <span>|</span>
        <p style="width: 484rpx;float: right;font-size: 28rpx;line-height: 45rpx;word-break:break-all;" @click="toFinishTask" :data-text="info.linkUrl" >
          {{info.linkUrl}}
          <!--(点击链接去完成任务）-->
          <span>(点击链接去完成任务）</span>
        </p>
...
<!--  </div>-->
  ...
  <div class="modalDialog" v-if="changeModel2">
    <div class="modalContent2" >
      ...
      <textarea v-model="backInfo.backoutRmk" placeholder="请输入撤销原因"></textarea>
    </div>
    <div class="modalFooter">
...
<script>
  export default {
    ...
    methods:{
      toFinishTask(e){
        if(!this.zfwTaskPersLinkVoId && this.until.TimeStep(this.info.endTm)>0){
          wx.showToast({
            title: '已经过了截止时间', // 标题
        ...
        if(this.info.taskPlat=='微信'){
          let url = '/pages/wxDetail/main?url='+encodeURIComponent(this.info.linkUrl) + '&postInfo='+ JSON.stringify(this.postInfo)
          wx.redirectTo({url})
        }
