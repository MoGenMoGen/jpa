  ...
  <div class="header">
    <ul v-if="dataShow">
      ...
      <li>
          <p class="hd">完成时间</p>
          <span>|</span>
          <p class="ct">{{info.endTm?info.endTm:''}}</p>
      </li>
<!--      <li>-->
<!--        <p class="hd">截图数量</p>-->
<!--        <span>|</span>-->
...
<!--      </li>-->
      ...
      <li>
        <p class="hd">任务对象</p>
        <span>|</span>
        <p class="ct"><span v-for="(v,s) in info.zfwUnitVoList" :key="s">{{v.unit}}</span></p>
      </li>
      <li>
        <p class="hd">任务来源</p>
        <span>|</span>
...
<!--      </li>-->
      ...
      <li>
        <p class="hd">链接地址</p>
        <span>|</span>
        <p style="width: 484rpx;float: right;font-size: 28rpx;line-height: 45rpx;word-break:break-all;" @click="toPage('/pages/wxDetail/main?url='+encodeURIComponent(info.linkUrl))">
          {{info.linkUrl}}
          (点击链接去完成任务）
        </p>
      </li>
      <li>
        <p class="hd">任务状态</p>
        <span>|</span>
        <p class="ct" v-if="info.status == 0">{{'已保存'}}</p>
        <p class="ct" v-if="info.status == 10">{{'待审批'}}</p>
        <p class="ct" v-if="info.status == 20" style="color: #2983E7;">{{'发表完成'}}</p>
        <p class="ct" v-if="info.status == 30" style="color: #2983E7;">{{'审批未通'}}</p>
        <p class="ct" v-if="info.status == 40" style="color:#E33637;">{{'已撤销'}}</p>
      </li>
      <!-- <li style="height: 220rpx;" v-if="cd==2">
        <img class="need" v-for="(i,index) in img" :key="index" :src="i" @click="toLook(i)"/>
      </li> -->
<!--      <li v-if="cd==2">-->
<!--        <p class="hd">完成时间</p>-->
<!--        <span>|</span>-->
...
<!--      </li>-->
      <li v-if="info.status == 40">
        <p class="hd" style="font-weight: 400;">撤销原因</p>
        <span>|</span>
        <p class="ct">{{info.backoutRmk?info.backoutRmk:''}}</p>
      </li>
      <li v-if="info.status == 40">
        <p class="hd" style="font-weight: 400;">撤销时间</p>
        <span>|</span>
        <p class="ct">{{info.backoutTm?info.backoutTm:''}}</p>
      </li>
      <li v-if="info.noPassRmk">
        <p class="hd" style="font-weight: 400;">审批意见</p>
        <span>|</span>
  ...
  export default {
    data(){
      return{
        ...
        // },
        // image:[],
        // posImgUrl:'http://oss-cn-hangzhou-zwynet-d01-a.internet.cloud.zj.gov.cn/pfyf/d7a30f5f00fa4efd846f499b47ea43fd.png'
      }
    },
    onLoad(options){
      // this.image = []
    ...
    onShareAppMessage (res) {
      return {
        title: '发布详情',
        desc: "发布详情页",
        path: '/pages/task/pubDetail/main?cd='+this.cd+'&id='+this.id
      }
    },
    onShow(){
    },
    methods:{
      ...
      //   let img = await this.api.upLoad(imgUrl)
      //   this.image.push(img)
      // },
      // //删除当前选中图片
      // del(e,index){
