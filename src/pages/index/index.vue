<template>
  <div class="home" @touchstart='touchStart' @touchend="touchEnd" @touchmove="touchMove">
    <!--标题以及下拉框-->
    <frames @select="updateCity"></frames>
<!--      <span></span><p>浙江社会治理融媒云手机版</p>-->
    <!--首页导航栏-->
    <div class="nav" :class="{Aclass:navList.length <= 6,Bclass:navList.length>6}">
    ...
    <div class="main">
      ...
      <div class="content">
        <!-- cd==1 热榜-->
        <div class="sort1" v-for="(item,index) in newsList" v-if="cd == 1" @click="toDetail(item)">
          <div class="image" :style="{backgroundImage:'url('+ item.imgUrl+')'}">
            <div>
          ...
          <p class="nm"><span style="font-weight:bold;">热门</span>{{item.nm}}</p>
        </div>
        <!-- 普通排列-->
        <div class="sort2" v-if="cd!=1 && showType==10" v-for="(item,index) in newsList" :key="index" @click="toDetail(item)">
          <div>
            <p>{{item.nm}}</p>
            <p><span>{{item.newsSrc}}</span>{{item.releTm}}</p>
          </div>
          <div>
            <img :src="item.imgUrl"/>
          </div>
        ...
        <!-- 图集-->
        <div class="sort3" v-if="cd!=1 && showType==20" v-for="(item,index) in newsList" :key="index" @click="toDetail(item)">
          <p>{{item.nm}}</p>
          <image :src="item.imgUrl" mode="aspectFill"></image>
          <p class="from"><span>{{item.newsSrc}}</span>{{item.releTm}}</p>
        </div>
      </div>
